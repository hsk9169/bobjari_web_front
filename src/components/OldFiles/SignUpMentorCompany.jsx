...
const SignUpMentorCompanyComp = (props) => {
    ...
    const handleAddJob = event => {
        ...
        if (event.target.innerText &&
            state.company.indexOf(event.target.innerText) === -1) {
    ...
    useEffect( () => {
        if (!state.initialized) { 
            if (props.location.data.company !== undefined) {
                if (props.location.data.company) { 
                    setState({
                        initialized: true,
    ...
    return (
            ...
            <List>
                <TransitionGroup>
                    {state.company ?
                        <Collapse>
                            <Box sx={{ px: 6, mt: 2 }}>
                                <Chip label={state.company} onDelete={handleRemoveCompany} />
                            </Box>
                        </Collapse>
                    : null}
                </TransitionGroup>
        ...
        {state.company.length > 0 ? <Button className={classes.root} onClick={handleNext} 
